...
function payOrder(){
	var key = $api.getStorage('key');
	var pay_sn = api.pageParam['pay_sn'];
	
	api.ajax({
	    url: ApiUrl + '/index.php?act=member_tr_buy&op=order_info&key='+key+'&pay_sn='+pay_sn,
	    method: 'get',
	    dataType: 'json',
	    data: {
    	...
    	if(ret.datas.data.stats==1){
			payData['subject'] = ret.datas.trip_info['goods_name'];
			payData['body'] = ret.datas.trip_info['goods_jingle'];
			payData['amount'] = ret.datas.trip_info['order_amount'];
			payData['tradeNO'] = ret.datas.trip_info['pay_sn'];
...
function payList(key,pay_sn){
	api.ajax({
	    url:ApiUrl + '/index.php?act=member_tr_buy&op=pay',
	    method: 'post',
	    dataType: 'json',
	    data: {
		    values: {
