var dynamicId,photosArr,chatBox,replyTotal,laudTotal,emotionData,laudLock=false;
...
function getData(){
	api.ajax({
    	...
    	if (ret) {
            ...
            reply(jsonData.datas.travels_info['userid'],jsonData.datas.travels_info['user_name']);
        } else {
            if(err.code!=3){
                api.toast({
                    msg: err.msg,
