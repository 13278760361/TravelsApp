function getData(){
	var dataUrl = "/index.php?act=member_collect&op=collectList&key="+key;
	ajaxRequest(dataUrl,'GET','',function(ret,err){
		if(ret){
			var retd = ret.datas.collect_list;
			var content = $api.byId('collect-content');
            var tpl = $api.byId('collect-template').text;
...
api.confirm({
        ...
        buttons:['确定', '取消']
            ...
            var url = '/index.php?act=member_collect&op=collectDelete&trip_id='+tripId+'&key='+key;
